...
- **`agentMissionRuntime.test.js` failures (pre-existing drift in read-only eve.js):** 4 assertions about **remote vs. in-person mission acceptance and courier cargo-staging** — the exact semantics that decide whether a browser (a remote, non-co-located client) may accept/complete a courier mission. Failing assertions (eve.js `server/tests/agentMissionRuntime.test.js`): `:3487` "fetch-family mission payloads … remote-offerable but non-remote-completable journal flags" (`1 !== true`), `:3524` "offered encounter missions are advertised as remotely acceptable" (`1 !== true`), `:3560` "courier missions that stage cargo on accept are not remotely acceptable" (`0 !== false`), `:3594` "a remote accept for an in-person-only courier mission is refused and left offered". Not fixed here (eve.js is read-only this goal). Flagged for **R4** — see gap G3. **Orchestrator re-diagnosis (2026-07-18 review): all four are test-side bugs, not runtime behavior.** Three assert strict booleans against the journal row's established `? 1 : 0` integer convention (`buildMissionJournalRow`, `agentMissionRuntime.js:4950-4954`, matching `importantMission` at `:4944`) while agreeing on semantics; the fourth checks `actionTuple[0]` (the dialogue token, 816) for the button ID that lives at `actionTuple[1]` (constants `:97`/`:108`) and can never pass. The runtime's refusal of the remote accept behaves correctly (`:6874-6887`). Introduced 2026-07-17 in eve.js `ed442548`; plausibly never passed. Fix via the G3 goal prompt (test-side, pending a wire-parity check on int-vs-bool).
...
| --- | --- | --- | --- | --- | --- |
| **G1** | **No server-owned travel/autopilot orchestrator** | 7–9 | **High** | **R5** | EveJS has every atomic travel command (`beyonce.CmdWarpToStuffAutopilot`/`CmdFollowBall`/`CmdStargateJump`/`CmdDock`, `structureJumpBridgeMgr.CmdJumpThroughStructureStargate`), but the 2-second decide-loop that sequences warp→approach→jump→dock lives in the **client** (`autopilot.py AutoPilot.Update`). `beyonceService.js:2901-2902` explicitly treats autopilot navigation as client-local. **(Superseded — see Direction below: the decide-loop stays client-side; the browser ports it and EveJS gains no travel-job code. Row kept so nobody rediscovers this as server work.)** |
| **G2** | **Route/destination state is client-only** | 7–8 | **High** | **R5** | `starmap.SetWaypoints` writes the `autopilot_waypoints` char-UI setting and solves the path with the **client-local** `clientPathfinderService` (`starMapSvc.py:2363`). No `RemoteSvc` stores waypoints or destination. **(Superseded — see Direction: route + pathfinding stay client-side in the browser, exactly as retail; there is no retail call to mirror and none is needed.)** |
| **G3** | **Courier remote-acceptance semantics partial / failing** | 3, 11 | **High** | **R4** | Accept/decline/complete all route through `agentMgr(bound).DoAction(actionID)` → `agentMissionRuntime.doAgentAction`, which is implemented — but the parity oracle currently fails 4 assertions on the journal `remotelyAcceptable/Completable` flags and the refusal path (see baseline). **Re-diagnosed at review: the runtime is correct; the 4 assertions are test bugs (int-vs-bool typing + a wrong tuple index).** Fix the tests via the G3 goal prompt so the oracle is trustworthy before R4. The in-person question still matters for R4: a browser character docked at the agent's station should count as in-person, not remote — validate when building Step 3/11. |
| **G4** | **`invbroker.SplitStack` missing** | 5 | Low | — | Right-click **Divide/Split stack** (`invItemFunctions.py:398`, on the manager moniker) has no `Handle_SplitStack` on invbroker. (A `Handle_SplitStack` DOES exist on `_other/contractProxyService.js:860` — service `contractProxy`, contracts flow, different 5-arg shape; it does **not** serve this call, so don't let a repo-wide grep count it as coverage.) Low impact: the drag-move path folds a partial quantity into `Add(qty)` / `MultiMerge(op qty)`, so moving a courier package needs no split call. Only an explicit "divide stack" action would fail. |
...
- **G1/G2 are not server work.** Autopilot and route/pathfinding stay client-side, exactly as in retail. The **browser** runs a port of `autopilot.py`'s ~2-second decide-loop and issues the atomic moves EveJS already handles (`beyonce.Cmd*`, `structureJumpBridgeMgr.CmdJumpThroughStructureStargate`); EveJS gets **no** travel-job code (the "server-owned travel job" idea is dropped). Closing the browser tab **is** closing the client: the loop stops and the ship finishes its last server-issued command then sits — faithful retail behavior, not a bug. The BFF only relays calls and must never drive movement with no client connected. So G1/G2 are client responsibilities we already have the calls for, not missing server handlers.
...
- **G3 is EveJS's own correctness item**, not client work — and orchestrator review re-diagnosed it as a **test-side** fix: the runtime behaves correctly; 3 tests assert booleans against the journal row's 1/0 integer convention and 1 checks the wrong tuple index (see baseline note). The G3 goal prompt fixes the tests (with a wire-parity check before choosing int vs bool). The design point stands: the browser-backed character is docked *at the agent's station*, so it should count as **in-person** (not a remote accept) — validate when building R4.

Net: the courier loop is buildable almost entirely against existing handlers; the real net-new work is **client-side** (the browser autopilot loop + page migrations), not server code.
...
| Open active-ship cargo (bind) | bound (`invbroker`) | `invbroker(mgr)→binding.GetInventoryFromId` | wire: `(itemid=shipID, passive=0)` — only these two cross the wire; the client wrapper's `locationID` (`invCache.py:537-554`) selects the moniker/session-check client-side and is never sent (`Handle_GetInventoryFromId` reads only `args[0]`) | origin `invCache.py:559`; ctrl `invControllers.py:638` | `invBrokerService.js:6082` — **Covered** |
...
| Set destination / waypoints | client-local | `starmap.SetWaypoints` → `SetCharUiSetting('autopilot_waypoints', …)` + `UpdateRoute()` | waypoints list | `…/ui/shared/maps/starMapSvc.py:2363` | **N/A (G2, reframed)** — no wire call; route solved by client-local `clientPathfinderService`. Route state stays client-side in the browser (see Direction); nothing to build server-side. |

---

## Step 8 — Undock and travel through every required gate (client-side autopilot: the browser runs the decide-loop)
...
| *(orchestration of the above)* | — | *client-side loop `AutoPilot.Update` / `NavigateSystemTo`* | — | `autopilot.py:274`, `:488` | **Client-side by design (G1, reframed)** — the browser ports this loop (R5); no server travel job exists or will |
...
| **Wallet journal** (mission ISK reward entry) | server | `account.GetTransactions` | `(accountingKeyCash, year, month, False)` | `…/services/accountsvc.py:116` | `accountService.js:666` — **Covered** |
| Journal ref-type names | server | `account.GetEntryTypes` | `()` | `…/services/accountsvc.py:89` | `accountService.js:574` — **Covered** |
| Corp wallet divisions header | server | `account.GetWalletDivisionsInfo` | `()` | `…/services/accountsvc.py:135` | `accountService.js:593` — **Covered** |
...
Tier note (review correction): the three `account` journal reads above are plain **server**-tier — the client accessor returns bare `sm.RemoteSvc('account')` with no bind step (`accountsvc.py:69-74`; its variable is merely *named* `moniker`). Do not attempt a `MachoBindObject` for `account`.
...
| `charMgr` | `character/charMgrService.js` *(registered but unused by the courier path — kept for completeness; do not whitelist from this row)* |
