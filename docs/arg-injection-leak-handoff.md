...
## Addendum (R99, 2026-07-23): alliance/war/corp-station WRITES — a WAR-STATE arg-injection family flagged (warRegistry accept/decline/retract/set act on caller-supplied warNegotiationID/warID with no session-party gate) + UpdateAlliance reads args[0] as an allianceID; the alliance governance writes and PayBill/MoveCorpHQHere are session-scoped

R99 wired the Phase-4 top-level WRITES for allianceRegistry (10) + warRegistry (9) + corpStationMgr (1) = 20, all confirm-gated at the BFF. Each dispatches on the ordinary top-level `/call` seam (no `MachoBindObject` allowlisted); the acting alliance/war-entity/corp is meant to derive from the session. Every route was smoke-checked refuse-without-confirm (400 CONFIRMATION_REQUIRED) ONLY — **NO confirmed dispatch was fired on the live world** (no bill paid, no relationship deleted, no ally negotiation accepted, no surrender accepted/declined, no war retracted, no corp HQ moved). Static reading of `warRegistryService.js`, `warNegotiationRuntimeState.js`, `allianceRegistryRuntime.js`, `corpStationMgrService.js`:

- **⚠ WAR-STATE VECTOR (family) — `warRegistry.AcceptAllyNegotiation` / `AcceptSurrender` / `DeclineAllyOffer` / `DeclineSurrender` / `RetractWarAllyOffer` / `RetractMutualWar` / `SetOpenForAllies`.** Each reads a CALLER-SUPPLIED `warNegotiationID` (or `warID`) off `args[0]` and mutates that record with **no check that the session is a party to the war/negotiation**:
  - `Handle_DeclineAllyOffer` / `Handle_DeclineSurrender` (`warRegistryService.js:267/299`) look up `getNegotiationRecord(args[0])` and force `negotiationState = DECLINED` — a browser could decline ANY negotiation by id.
  - `Handle_RetractWarAllyOffer` (`:210`) sets any negotiation to RETRACTED by id.
  - `Handle_RetractMutualWar` (`:283`) calls `endWarWithReason(args[0], …)` — ENDS any war by id (the `retractedBy` is the session entity, but the war selector is caller-supplied, so a session can end a war it is not in).
  - `Handle_SetOpenForAllies` (`:315`) takes `(warID, state)` and flips `openForAllies` on any war — the handler doesn't even receive `session`.
  - **`Handle_AcceptAllyNegotiation` (`:261`) → `acceptAllyNegotiation(warNegotiationID, session)` (`warNegotiationRuntimeState.js:240`) is the most severe: it resolves the negotiation purely by caller-supplied id and debits the CONCORD ally fee + ally fee from the PAYER wallet derived from the negotiation record (`negotiation.ownerID2 / againstID`), NOT from the session** — so a caller who names a foreign entity's pending ally negotiation could trigger a spend against that entity's wallet and bind an ally to a war it is not party to. `Handle_AcceptSurrender` (`:293` → `acceptSurrender`) likewise resolves by id and ends the war / settles reparations without a session-party gate.
  - `CreateWarAllyOffer` / `CreateSurrenderNegotiation` (`:186/:226`) DO set `ownerID1 = resolveWarEntityID(session)` and require the war to exist, but still create a negotiation attached to a caller-supplied `warID` the session may not belong to (lower severity — the created record is owned by the session entity). Noted for provenance.
  - **Server fix (later):** gate each war verb on the session being a belligerent (or the negotiation's owner) for that `warID`/`warNegotiationID`; resolve the paying wallet / retracting entity from the session, not from the caller-named record.
- **⚠ `allianceRegistry.UpdateAlliance`** (`allianceRegistryRuntime.js:421`) resolves the target via `resolveAllianceIDFromArgs(args, session)` (`:151`), which reads `args[0]` as a possible `allianceID` before falling back to the session alliance. The BFF route passes `args = [description, url]`, so a **numeric-string description** would be coerced to a positive integer and steer the edit at a FOREIGN alliance's description/url. Every OTHER allianceRegistry write here uses `resolveAllianceIDFromSession` (args ignored) and is session-scoped. Server fix (later): `UpdateAlliance` should use `resolveAllianceIDFromSession` (or `resolveAllianceIDFromArgs([], session)`, as `DeclareWarAgainst` already does) so the description arg cannot double as an alliance selector.
- **SESSION-SCOPED (SAFE):**
  - `allianceRegistry.SetRelationship` / `DeleteRelationship` / `AddAllianceContact` / `AddBulletin` / `UpdateApplication` / `SetPrimeHour` / `SetCapitalSystem` / `DeclareExecutorSupport` all resolve the alliance from `resolveAllianceIDFromSession(session)` (args carry only the contact/relationship/bulletin/application/hour/system payload). A foreign contactID/toID/corporationID simply lands in (or misses) the session alliance's own tables — no foreign-alliance selector.
  - **`allianceRegistry.PayBill`** (`:927`) — the PAYER is `session.corporationID` (server-derived); `args[0]` is the `billID` to settle. `payBillFromCorporation(billID, sessionCorp, accountKey)` debits the SESSION corp, so a caller cannot make another corp pay. (Whether an unrelated `billID` can be paid by the session corp is a separate ledger-consistency question, not a foreign-entity write — the spender is the session.)
  - **`corpStationMgr.MoveCorpHQHere`** (`:313`) — `corporationID = resolveCorporationID(session)` (session corp); `stationID` from args/session. A browser can only move the SESSION corp's HQ.

**NOT exercised live (deliberate, fast-mode):** all 20 are confirm-gated and were smoke-checked refuse-without-confirm (400 CONFIRMATION_REQUIRED) ONLY; the confirmed-gate-passes path was shown via a benign governance route with no live session (→ 409 NO_LIVE_SESSION, no dispatch). NO confirmed dispatch was run for any route (the war verbs would change real war state / spend ISK; MoveCorpHQHere / PayBill are consequential/financial). Findings are a **static reading** of the handlers + runtime-state helpers, the same confidence level as the other static entries here. All 20 stay allowlisted + confirm-gated; none of the three `MachoBindObject` pairs is allowlisted.
