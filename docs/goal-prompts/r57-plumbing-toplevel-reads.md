...
7. **Un-stale every per-service refusal test AND every "exactly-this-set" enumeration (MANDATORY — R67 cleaned up the refusal lists; R76 caught an enumeration miss).** The central snapshot is NOT the only place that pins a service's surface. Two stale-test shapes exist:
   - **Refusal lists** — hardcoded "these methods are refused / this service is out of slice" (e.g. `webGatewayContracts`, `webGatewayAgentMgr`, `webGatewayCourierComplete`, `webGatewayMail`). A newly-allowlisted READ that one of these names goes RED (200 ≠ 403).
   - **Exact-surface enumerations** — `assert.deepEqual([...listedService].sort(), [ ...hardcoded ])` "the service's allowlisted surface is exactly X" (e.g. `webGatewayFlightVerbs`'s beyonce set — R71's `beyonce.GetFormations` broke it because R71 grepped only for "refused" and missed the `deepEqual`). A newly-allowlisted pair on such a service breaks the enumeration.

   For **every** method/service you allowlist, `grep -rn "<Method>"` AND `grep -rn "<serviceName>"` across `server/tests/webGateway*.test.js`, and check BOTH shapes: any refusal assertion (remove the now-allowed READ; keep still-refused WRITES asserted — never weaken a still-valid refusal) AND any `deepEqual`/"exactly"/enumeration of the service's allowlisted surface (add your new method in sorted position). Run every affected file via the isolated runner and confirm green before committing. Skipping this leaves the suite RED and silently claims "bridge-only held" while a test is failing.
