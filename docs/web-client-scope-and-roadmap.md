...
| R2 | Pending | **Spot-test milestone** (re-scoped 2026-07-18; character sheet/skills moved to a later goal): persistent browser-backed session in the gateway + `SelectCharacterID`, and the first page on the new stack (view-lib spike): login → character-selection UI → docked station panel (`GetStationInfo`/`GetStationItemBits`/`GetGuests`) | Operator can log in at `/dist/`, pick a character, and see the station they're docked in; in-process tests prove the flow; live run is the operator's spot test |
