...
test("R98 — a null-returning share/war write decodes to {ok, applied, result:null}", () => {
  const ack = decodeCorpRegistryShareWarWriteAck(plainAck({ ok: true, applied: true, result: null }));
...
test("R98 — a refused (unconfirmed) / role-refused write is not-applied, not a throw", () => {
  const ack = decodeCorpRegistryShareWarWriteAck(plainAck({ ok: true, applied: false }));
...
test("R98 — DeclareWarAgainst passes its war-record return through result untouched", () => {
  ...
  const ack = decodeCorpRegistryShareWarWriteAck(plainAck({ ok: true, applied: true, result: record }));
...
test("R98 — an ID-returning write surfaces its numeric id", () => {
  const ack = decodeCorpRegistryIdWriteAck(plainAck({ ok: true, applied: true, result: 900001 }));
  assert.deepEqual(ack, { ok: true, applied: true, id: 900001 });
});

test("R98 — the id ack reads null for a non-number / absent result", () => {
  assert.equal(decodeCorpRegistryIdWriteAck(plainAck({ ok: true, applied: true, result: null })).id, null);
  assert.equal(decodeCorpRegistryIdWriteAck(plainAck({ ok: true, applied: false })).id, null);
});

// --- KickOutMembers { kicked, notKicked } ack ---------------------------------

test("R98 — KickOutMembers splits kicked / notKicked id lists", () => {
  const result = plainAck({ kicked: [140000002, 140000003], notKicked: [140000009] });
