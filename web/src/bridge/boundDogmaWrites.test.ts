...
test("R101 — a PeelAndLink weapon-bank state dict is carried through untouched", () => {
  ...
  const ack = decodeDogmaModuleWriteAck(ackKeyVal({ ok: true, applied: true, result: bankDict }));
  assert.equal(ack.applied, true);
  assert.deepEqual(ack.result, bankDict);
});

test("R101 — an UnlinkModule peeledModuleID number return passes through", () => {
  const ack = decodeDogmaModuleWriteAck(ackKeyVal({ ok: true, applied: true, result: 7400000021 }));
  assert.equal(ack.applied, true);
...
test("R101 — a ChangeDroneSettings boolean return passes through", () => {
  const ack = decodeDogmaModuleWriteAck(ackKeyVal({ ok: true, applied: true, result: true }));
  assert.equal(ack.applied, true);
