...
test("R95 — a null-returning skill/clone write decodes to {ok, applied, result:null}", () => {
  const ack = decodeSkillCloneWriteAck(ackKeyVal({ ok: true, applied: true, result: null }));
...
test("R95 — a declined (unconfirmed / refused) write is not-applied, not a throw", () => {
  const ack = decodeSkillCloneWriteAck(ackKeyVal({ ok: true, applied: false }));
...
test("R95 — a write that returns a raw object passes result through untouched", () => {
  ...
  const ack = decodeSkillCloneWriteAck(ackKeyVal({ ok: true, applied: true, result: raw }));
...
test("R95 — SetSafetyLevel ack decodes the echoed new safety level", () => {
  const ack = decodeSafetyLevelWriteAck(ackKeyVal({ ok: true, applied: true, result: 2 }));
...
test("R95 — SetSafetyLevel reads a numeric-string level, and null when absent", () => {
  assert.equal(decodeSafetyLevelWriteAck(ackKeyVal({ ok: true, applied: true, result: "1" })).safetyLevel, 1);
  assert.equal(decodeSafetyLevelWriteAck(ackKeyVal({ ok: true, applied: true, result: null })).safetyLevel, null);
});

test("R95 — safety level 0 (disabled) is kept, not coerced to null", () => {
  assert.equal(decodeSafetyLevelWriteAck(ackKeyVal({ ok: true, applied: true, result: 0 })).safetyLevel, 0);
});
