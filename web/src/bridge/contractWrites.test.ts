...
test("decodeContractWriteAck reads ok/applied off the BFF envelope", () => {
  const ack = decodeContractWriteAck(ackKeyVal({ ok: true, applied: true, result: null }));
  assert.deepEqual(ack, { ok: true, applied: true });
});

test("decodeContractWriteAck is false for a non-keyval / empty response (never throws)", () => {
  assert.deepEqual(decodeContractWriteAck(null as unknown as JsonValue), { ok: false, applied: false });
  assert.deepEqual(decodeContractWriteAck(ackKeyVal({ ok: false, applied: false })), {
    ok: false,
    applied: false,
  });
});
...
test("decodeCreateContractAck surfaces the new contract id list", () => {
  ...
  const ack = decodeCreateContractAck(ackKeyVal({ ok: true, applied: true, result }));
...
test("decodeCreateContractAck yields an empty list when nothing was created", () => {
  const ack = decodeCreateContractAck(ackKeyVal({ ok: true, applied: true, result: null }));
...
test("decodeAcceptContractAck reads the contract id off the returned row", () => {
  const ack = decodeAcceptContractAck(
    ackKeyVal({ ok: true, applied: true, result: contractRow(8100) }),
  );
  assert.equal(ack.contractID, 8100);
});

test("decodeAcceptContractAck is 0 when the accept was declined (null row)", () => {
  const ack = decodeAcceptContractAck(ackKeyVal({ ok: true, applied: true, result: null }));
...
test("decodeDeleteMultipleContractsAck splits deleted vs failed", () => {
  ...
  const ack = decodeDeleteMultipleContractsAck(ackKeyVal({ ok: true, applied: true, result }));
